...
if [ ! -f ${c_root}/CMakeLists.txt ]; then
    colorecho $_red "ERROR: File ${c_root}/CMakeLists.txt does not exist"
    echo usage: $0 [c-root]
    echo e.g.: $0 ~/repos/c
    exit 1
fi
...
[ $? -eq 0 ] || { colorecho $_red "mkdir restbed failed"; exit 1; }

cd ${script_dir}/wrapper/deps/restbed
[ $? -eq 0 ] || { colorecho $_red "cd restbed failed"; exit 1; }

git clone https://github.com/Corvusoft/restbed .
[ $? -eq 0 ] || { colorecho $_red "clone restbed failed"; exit 1; }

git checkout 1b43b9a
[ $? -eq 0 ] || { colorecho $_red "git checkout failed"; exit 1; }

git submodule update --init --recursive
[ $? -eq 0 ] || { colorecho $_red "git submodule update failed"; exit 1; }


# -------------------------------------------------------------
# pull the tests out of the CMakeLists file
sed -i '/{PROJECT_SOURCE_DIR}\/test\//d' CMakeLists.txt
[ $? -eq 0 ] || { colorecho $_red "sed failed"; exit 1; }
...
[ $? -eq 0 ] || { colorecho $_red "mkdir ${script_dir}/wrapper/build failed"; exit 1; }

cd ${script_dir}/wrapper/build
[ $? -eq 0 ] || { colorecho $_red "cd ${script_dir}/wrapper/build failed"; exit 1; }
...
[ $? -eq 0 ] || { colorecho $_red "cmake failed"; exit 1; }

make edge_e2e_rest_server -j 4
[ $? -eq 0 ] || { colorecho $_red "make failed"; exit 1; }

colorecho $_green SUCCESS
colorecho $_green To build, run 'make edge_e2e_rest_server' in $(pwd)
