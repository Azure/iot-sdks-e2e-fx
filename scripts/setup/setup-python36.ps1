...
$py = Invoke-PyCmd "-m pip install setuptools"; Invoke-Expression  $py
$py = Invoke-PyCmd "-m pip install --user -e python_glue"; Invoke-Expression  $py
if($LASTEXITCODE -ne 0)
{
    Write-Host "user path not accepted.  Installing globally" -ForegroundColor Yellow
    $py = Invoke-PyCmd "-m pip install -e python_glue"; Invoke-Expression  $py
    if($LASTEXITCODE -ne 0)
    {
...
set-location $root_dir
$py = Invoke-PyCmd "-m pip install --user -e horton_helpers"; Invoke-Expression  $py
if($LASTEXITCODE -ne 0)
{
    Write-Host "user path not accepted.  Installing globally" -ForegroundColor Yellow
    $py = Invoke-PyCmd "-m pip install -e horton_helpers"; Invoke-Expression  $py
    if($LASTEXITCODE -ne 0) {
...
$py = Invoke-PyCmd "-m pip install --user -r requirements.txt"; Invoke-Expression  $py
if($LASTEXITCODE -ne 0) {
    Write-Host "user path not accepted.  Installing globally" -ForegroundColor Yellow
    $py = Invoke-PyCmd "-m pip install -r requirements.txt"; Invoke-Expression  $py
    if($LASTEXITCODE -ne 0) {
